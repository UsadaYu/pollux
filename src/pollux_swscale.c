...
pollux_api int pollux_sws_scale(pollux_sws_handle handle, pollux_frame_t *src,
  ...
  frame_t *rf = (frame_t *)(dst->priv_data);
  if (!rf)
    goto label_error;
  ...
  struct SwsContext *sc = (struct SwsContext *)handle;
  dst->height = sws_scale(sc, (const uint8_t *const *)(src->data),
...
pollux_api pollux_sws_handle pollux_sws_context_alloc(
  ...
  struct SwsContext *sc =
    sws_getContext(src_width, src_height, src_av_fmt, dst_width, dst_height,
                   dst_av_fmt, flags, src_filter, dst_filter, param);
  if (!sc) {
