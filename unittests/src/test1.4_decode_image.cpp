...
const auto OUT_PREFIX = std::string(test_generated_pre) + "1.4_";
...
class PolluxDecoder {
 public:
  ...
  void param_set(const std::string_view &url,
    ...
    auto ret = pollux_->param_set(pollux_, url.data(), args);
    if (ret) {
...
int main() {
  ...
  try {
    ...
    for (size_t i = 0; const auto &file : INPUT_FILE_ARR) {
      ...
      std::string file_name = OUT_PREFIX + std::to_string(i++) + "_" +
        std::to_string(rst->linesize[0] / 3) + "-" +
        std::to_string(rst->height) + ".rgb24";
