...
static inline void test_init() {
  ...
  gfd =
#ifdef _WIN32
      _sopen(test_log_path, _O_RDWR | _O_CREAT | _O_APPEND,
             _SH_DENYNO, _S_IREAD | _S_IWRITE);
#else
      open(test_log_path, O_RDWR | O_CREAT | O_APPEND,
           S_IRUSR | S_IWUSR);
#endif
  if (-1 == gfd) {
...
/**
 * @brief Recursively create a directory; if it already
 *  exists, clear its content.
 *
 * @param[in] input_path: The original path (which can be
 *  a relative or absolute path).
 *
 * @return std::filesystem::path, the final directory
 *  path after creation or clearing
 *
 * @throws std::runtime_error, thrown when the operation
 *  fails.
 */
std::filesystem::path rmkdir(const std::string& dir) {
  try {
