...
/**
 * @note Considering that some image viewing software does
 *  not support high-resolution GIFs well, do not set the
 *  length and width too large here.
 */
constexpr int WIDTH = 1763, HEIGHT = 1100;
constexpr auto INPUT_FILE = "./input2_3506-2200_video.avi";
const auto OUTPUT_FILE = std::string(test_generated_pre) +
                         "2.4_" + std::to_string(WIDTH) +
                         "-" + std::to_string(HEIGHT) +
                         ".gif";

constexpr auto CONT_FMT =
    pollux_cont_fmt_t::pollux_cont_fmt_gif;
constexpr auto CODEC_ID =
    pollux_codec_id_t::pollux_codec_id_gif;
...
int main() {
  ...
  try {
    ...
    PolluxEncoder encoder(OUTPUT_FILE);
