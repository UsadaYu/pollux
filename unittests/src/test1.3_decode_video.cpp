...
constexpr int ING_GROUP = 404;
constexpr int T_GROUP = 5;
...
using PolluxFramePtr = std::unique_ptr<pollux_frame_t, PolluxFrameDeleter>;

class PolluxDecoder {
 public:
  PolluxDecoder() {
    if (pollux_decode_init(&pollux_)) {
      throw std::runtime_error("pollux_decode_init failed");
    }
  }
  ...
  PolluxDecoder(PolluxDecoder &&other) noexcept : pollux_(other.pollux_) {
    other.pollux_ = nullptr;
  }
  PolluxDecoder &operator=(PolluxDecoder &&other) noexcept {
    if (this != &other) {
      if (pollux_) {
        pollux_->release(pollux_);
        pollux_decode_deinit(pollux_);
  ...
  void param_set(const std::string &url, pollux_decode_args_t *args) {
    int ret = pollux_->param_set(pollux_, url.c_str(), args);
    if (ret) {
      throw std::runtime_error(
        std::format("param_set failed with code: {}", ret));
    }
  }
  ...
  void seek_file(int64_t timestamp, int64_t stream_index, int64_t flags) {
    if (pollux_->seek_file(pollux_, timestamp, stream_index, flags)) {
...
void file_write(PolluxDecoder &decoder) {
  ...
  std::mt19937 gen(rd());
  std::uniform_int_distribution<> rand_count_dist(1, T_GROUP - 1);
  std::uniform_int_distribution<> action_dist(0, T_GROUP - 2);

  for (int i = 0; i < ING_GROUP; ++i) {
    ...
    for (size_t j = 0; j < frames.size(); ++j) {
      ...
      default:
        ...
        std::string file_name =
          std::format("{0}/{1}-{2}_{3}_{4}.nv12", OUT_DIR, rst->linesize[0],
                      rst->height, i, j);
        ...
        ofs.write(reinterpret_cast<const char *>(rst->data[0]),
                  static_cast<std::streamsize>(rst->linesize[0]) * rst->height);
        ofs.write(
          reinterpret_cast<const char *>(rst->data[1]),
          (static_cast<std::streamsize>(rst->linesize[1]) * rst->height) >> 1);
        break;
      }
    }
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    pollux_img_t img = {1280, 720, 16, pollux_pix_fmt_nv12};
