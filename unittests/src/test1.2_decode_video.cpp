...
constexpr size_t IMG_GRP = 128;

constexpr auto INPUT_FILE_1 =
    "./input1_2560-1440_video.mp4";
constexpr auto INPUT_FILE_2 =
    "./input2_3506-2200_video.avi";

constexpr char name_[] = "1.2_";
...
class PolluxDecoder {
 ...
 private:
  std::string fmt_to_file_suffix(
      pollux_pix_fmt_t fmt) const {
    static const std::map<pollux_pix_fmt_t, std::string>
        suffix = {{pollux_pix_fmt_yuv444p, ".yuv444p"},
  ...
  bool write_frame_to_file(
      const pollux_pix_fmt_t fmt, pollux_frame_t* rst,
...
#undef W
  ...
  int process_frames(const pollux_pix_fmt_t fmt) {
    ...
    for (unsigned int i = 0; i < IMG_GRP; i++) {
      ...
      std::string file_name =
          OUT_DIR + "/" + std::to_string(n) + "_" +
          std::to_string(rst->linesize[0]) + "-" +
          std::to_string(rst->height) + "_" +
          std::to_string(i) + fmt_to_file_suffix(fmt);
 ...
 public:
  int set_params_and_decode(pollux_img_t* img,
                            const std::string& file) {
    pollux_decode_args_t args = {16, img};

    int ret =
        pollux_->param_set(pollux_, file.c_str(), &args);
    if (ret) {
    ...
    pollux_pix_fmt_t fmt =
        img ? img->fmt : pollux_->stream.video_img_fmt;
